...
import os
import sys
sys.stdout=open('errors_RNNseq2seq_jet_2dis.txt','w')
...
# 打开文件
with open('result_truth.txt', 'r') as file:
    # 读取文件内容
    content = file.read()
    # 分割内容为行
...
with open('result_jet.txt', 'r') as file:
    # 读取文件内容
    content = file.read()
    # 分割内容为行
    lines = content.strip().split('\n')
...
with open('result_predict.txt', 'r') as file:
    # 读取文件内容
    content = file.read()
    # 分割内容为行
    lines = content.strip().split('\n')
